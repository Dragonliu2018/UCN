...
1. 真值转编码：将真值转为二进制科学计数法，填写**Sign**\(单1双1，正0负1\)，指数+Bias\(单127双1023\)填写**exp**\(单8双11\)，**frac**照搬小数部分\(单23双52，末尾补0\)
2. 编码转真值：**sign**判断正负，**exp**-Bias得指数，**frac**根据exp转成二进制小数再转化\(frac转成十进制小数再乘以exp\)
