...
某计算机主存最大寻址空间为4GB，按字节编址，假定用64M×8位的具 有8个位平面的DRAM芯片组成容量为512MB、传输宽度为64位的内存 条。请回答下列问题： 

1. 每个内存条需要多少个DRAM芯片？
2. 构建容量为2GB的主存时，需要几个内存条？
3. 主存地址共有多少位？其中哪几位用作DRAM芯片内地址？哪几位为 DRAM芯片内的行地址？哪几位为DRAM芯片内的列地址？哪几位用于 选择芯片？

#### 解答

1. 512MB / 64MB = 8
2. 2GB / 512MB = 4
3. 主存地址：4G = 2^32B，32位 位扩展 -&gt; 交叉编址 -&gt; 片内地址+片号 -&gt; 内存条\(3\)+行\(13\)+列\(13\)+片号\(3\)
...
* **答**：
  ...
  * 分页式虚拟存储器
    ...
    * 物理地址 = 物理页号 + 页内地址（PA = PPN + PPO\( CI + CO\)）
    * TLB全相联：VA = tag + VPO
  ...
  * 主存储器
    * 片内连续编址方式：主存地址：\[片选地址\]+\[片内地址\] 即 \[片号\]+\[行号\]+\[列号\]
    * 交叉编址方式: 主存地址：\[片内地址\]+\[片选地址\] 即 \[行号\]+\[列号\]+\[片号\]
...
* **答**：
  ...
  * 原则上物理内存空间的大小是取决于地址总线的位数。但是地址线位数的增加，还需要相关技术的支撑！比如我们课上学到的页表项一般占4B，如果地址线增加到36位，页大小还是4KB的话，4B的页表项就不够用了，因为去掉一些支撑操作的位，最多只够放23位物理页号，而36位物理内存，4KB页大小的话，物理页号是24位。怎么办？两种办法，一个是将页大小增大，另一个是让页表项占更多字节。PSE技术采用前者，大家都提到的PAE技术采用的是后者，就是把页目录项和页表项扩充为8B，IA32原来的两级页表也被转化成了三级页表（保证页目录表和子页表是4KB），而且支持4KB和2MB两种页大小。而到了x86-64，很自然地变成了四级页表。课上讲的酷睿i7，是52位地址线，最大支持4PB内存，虽然虚拟地址只有48位。这里，大家一定要把虚拟地址空间和物理内存空间区分开。 至于操作系统对内存大小的影响，因为页表是由它来维护的，它当然可以限制地址转换时物理页号的位数了。而操作系统的一部分会常驻内存，也占掉了一部分空间。[x86的pae，pse和×86](http://www.360doc.com/content/13/1109/12/1956846_327801619.shtml)

* **问**：自己电脑配的是4G的内存条，可是装完系统之后发现电脑上显示的只有3.2G左右可用内存，其它的内存跑到哪去了？
* **答**：在使用计算机时，其最大支持的内存是由 操作系统 和 硬件 两方面决定的。
  * 在计算机中 CPU的地址总线数目决定了CPU 的寻址范围，这种由地址总线对应的地址称作为物理地址。假如CPU有32根地址总线（一般情况下32位的CPU的地址总线是32位，也有部分32位的CPU地址总线是36位的，比如用做服务器的CPU），那么提供的可寻址物理地址范围为 $$2^{32}=4 \mathrm{GB}$$ （在这里要注意一点，我们平常所说的32位CPU和64位CPU指的是CPU一次能够处理的数据宽度，即位宽，不是地址总线的数目）。
...
* **参考**：[32位系统最大只能支持4GB内存之由来](https://www.cnblogs.com/dolphin0520/archive/2013/05/31/3110555.html)
